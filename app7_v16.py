...
import yaml # To generate schema.yml files
                ...
                try:
                    ...
                    if source_type == "SQL File":
                        ...
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            metadata_generator = Agent(role="DBT Metadata Generator", goal="Generate high-quality DBT documentation and tests for the converted Snowflake models.", backstory="An expert in dbt development, specializing in creating `schema.yml` files with detailed column descriptions and appropriate data quality tests (e.g., `not_null`, `unique`).", llm=custom_llm, verbose=True)
                            ...
                            try:
                                ...
                                oracle_logic_summary = crew_output.tasks_outputs[0]
                                metadata_content_raw = crew_output.tasks_outputs[2]
                                final_sql_output = crew_output.tasks_outputs[4]
                                ...
                                metadata_path = os.path.join(dbt_path, "models", "schema.yml")
                                ...
                                if session:
                                    try:
                                        ...
                                        explain_result = session.sql(f"EXPLAIN {converted_sql}").collect()
                                        explain_plan_content = "\n".join([f"{row[0]}: {row[1]}" for row in explain_result])
                                    except Exception as e:
                    ...
                    with open(metadata_path, "rb") as f:
                        st.download_button(label=f"⬇️ Download `schema.yml`", data=f, file_name="schema.yml", mime="text/yaml")
                    
                    st.success(f"✅ Saved `{safe_name}.sql` and `schema.yml` to `{output_dir}`")
                except Exception as e:
