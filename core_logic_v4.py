...
def run_crew_migration(file_content, source_type, model_type, custom_llm):
    ...
    # A more robust way to extract the summary and final SQL from the full output string.
    final_output_str = final_output if isinstance(final_output, str) else str(final_output)
    
    # Use regex to find the output from the Oracle Analyst task
    oracle_analyst_pattern = r'### Task: .*?\n\n### Agent: Oracle PL/SQL Analyst\n\n### Task Output:\n(.*?)(?=\n### Task: |\Z)'
    summary_match = re.search(oracle_analyst_pattern, final_output_str, re.DOTALL)
