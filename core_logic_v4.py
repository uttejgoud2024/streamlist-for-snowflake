...
def run_crew_migration(file_content, source_type, model_type, custom_llm):
    ...
    # A more robust way to extract the summary and final SQL
    oracle_logic_summary = "No summary available."
    if isinstance(final_output, list) and len(final_output) > 0:
        oracle_logic_summary = str(final_output[0]).strip()
        if not oracle_logic_summary:
             oracle_logic_summary = "No summary was generated by the Oracle PL/SQL Analyst."
    elif isinstance(final_output, str):
        # Fallback to regex if the output is a single string
        oracle_analyst_pattern = r'### Task: .*?\n\n### Agent: Oracle PL/SQL Analyst\n\n### Task Output:\n(.*?)(?=\n### Task: |\Z)'
        summary_match = re.search(oracle_analyst_pattern, final_output, re.DOTALL)
        if summary_match:
            oracle_logic_summary = summary_match.group(1).strip()
    
    # Extract SQL from the markdown block
    clean_sql = ""
    sql_match = re.search(r"```sql\s*(.*?)\s*```", str(final_output), re.DOTALL)
    if sql_match:
        clean_sql = sql_match.group(1).strip()
    else:
        # Fallback to the last part of the output if no markdown block is found
        clean_sql = str(final_output).split("Final Answer:")[-1].strip()
