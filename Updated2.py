                        ...
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            try:
                                ...
                                # Access the final output string from the CrewAI result object
                                ...
                                # Use a simple string split to extract the code block
                                start_marker = "```sql"
                                end_marker = "```"
                                
                                if start_marker in final_output_str:
                                    # Get the content after the start marker
                                    sql_content = final_output_str.split(start_marker, 1)[1]
                                    # Get the content before the end marker
                                    clean_sql = sql_content.split(end_marker, 1)[0].strip()
                                else:
                                    # Fallback to the entire string if no markdown block is found
