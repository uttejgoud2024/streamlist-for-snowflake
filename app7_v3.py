...
class SnowflakeCortexLLM(BaseLLM):
    def __init__(self, sp_session: Session, model: str = "llama3.1-8b", temperature: float = 0.7, region_name: str = 'us-west-2'):
    ...
    def call(self, messages: Union[str, List[Dict[str, str]]], **kwargs) -> str:
        try:
            ...
            result_df = self.sp_session.sql(
                f"SELECT SNOWFLAKE.CORTEX.AI_COMPLETE(model => '{self.model}', prompt => '{safe_prompt}', region => '{self.region_name}')"
            ).collect()
...
def get_snowpark_session_and_llm():
    try:
        ...
        # You may need to change 'us-west-2' to the correct region for the model
        llm = SnowflakeCortexLLM(sp_session=session, region_name='us-west-2')
