...
# Setup logging for a cleaner console output
            ...
            for file in uploaded_files:
                try:
                    ...
                    if source_type == "SQL File":
                        ...
                        # Progress tracking for each CrewAI task
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            status.update(label="🕵️ **Step 1/4: Analyzing Oracle logic...**", state="running")
                            ...
                            status.update(label="🤖 **Step 2/4: Translating to Snowflake SQL...**", state="running")
                            ...
                            status.update(label="⚙️ **Step 3/4: Optimizing query for Snowflake...**", state="running")
                            ...
                            status.update(label="✅ **Step 4/4: Final review and validation complete...**", state="running")
                            ...
                            try:
                                ...
                                # More robust parsing logic to handle different AI output formats
                                clean_sql = ""
                                ...
                                # FIX: Decode the literal escape sequences before conversion
                                ...
                                # Final check for remaining Oracle syntax and wrap the model
                                converted_sql = convert_oracle_to_snowflake(clean_sql)
                                wrapped_sql = wrap_sql_in_dbt_model(converted_sql, model_type)
                                
                                status.update(label="✅ **Migration completed!**", state="complete", expanded=False)
                            except Exception as e:
