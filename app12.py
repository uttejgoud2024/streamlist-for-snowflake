...
""")
    ...
    run_full_workflow = st.checkbox("Run Full DBT Workflow (seed → run → test → snapshot → docs generate → build)")
        ...
        if dbt_path:
            if run_full_workflow:
                commands = [
                    ...
                    f"dbt snapshot --project-dir {dbt_path}",
                    f"dbt docs generate --project-dir {dbt_path}",
                    f"dbt build --project-dir {dbt_path}"
                ]
                for cmd in commands:
