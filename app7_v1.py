...
with tab3:
    ...
    if st.button("🚀 Convert and Save Models"):
        if uploaded_files and output_dir:
                ...
                for file in uploaded_files:
                    try:
                        ...
                        if source_type == "SQL File":
                            ...
                            with st.spinner(f"Using CrewAI to convert `{file.name}`..."):
                                oracle_analyst = Agent(role="Oracle PL/SQL Analyst", goal="Analyze and explain the logic of Oracle procedures.", backstory="A seasoned expert in Oracle PL/SQL, meticulously breaking down complex business logic.", llm=custom_llm, verbose=True)
                                dbt_modeler = Agent(role="Snowflake DBT Modeler", goal="Translate Oracle procedural logic into clean, efficient DBT models.", backstory="A master of Snowflake SQL and DBT best practices, focusing on modularity and performance.", llm=custom_llm, verbose=True)
                                snowflake_optimizer = Agent(role="Snowflake Optimizer", goal="Refactor and optimize the converted SQL for Snowflake's architecture.", backstory="A performance engineer with deep knowledge of Snowflake's query engine, ensuring all code runs at peak efficiency.", llm=custom_llm, verbose=True)
                                quality_reviewer = Agent(role="SQL Quality Reviewer", goal="Validate the final DBT model for correctness, formatting, and adherence to standards.", backstory="A meticulous reviewer who ensures the final output is production-ready.", llm=custom_llm, verbose=True)

                                task1 = Task(description=f"Analyze and document the business logic and dependencies of the following Oracle {source_type}:\n\n{file_content}", expected_output=f"A clear, structured document explaining the {source_type.lower()}'s logic, including inputs, outputs, and core functions.", agent=oracle_analyst)
                                task2 = Task(description=f"Based on the analysis, convert the procedural logic into a single DBT model SQL file for Snowflake. The output should be a single, executable SQL query, without any procedural constructs. The SQL should be for a {model_type}.", expected_output="A single, well-formatted DBT model SQL file (a SELECT statement) that can be run on Snowflake.", agent=dbt_modeler)
                                task3 = Task(description="Given the converted DBT model SQL, review and apply optimizations for Snowflake's columnar storage and query engine. Use techniques like `QUALIFY` and `ROW_NUMBER` where appropriate.", expected_output="An optimized DBT model SQL file.", agent=snowflake_optimizer)
                                task4 = Task(description="Review the final DBT model SQL. Check for syntax correctness, adherence to Snowflake SQL best practices, and overall code quality.", expected_output="The final, production-ready DBT model SQL, formatted with correct indentation and comments.", agent=quality_reviewer)
            ...
            st.warning("⚠️ Please upload at least one file and provide a valid DBT path. If using AI migration, ensure Snowflake session is active.")
