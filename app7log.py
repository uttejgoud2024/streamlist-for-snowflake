...
def create_summary_file(output_dir, file_name, converted_sql, model_type, oracle_logic_summary):
...
with tab3:
    ...
    if dbt_path:
        ...
        # --- Configure logging to a file in the new directory ---
        logging.basicConfig(
            level=logging.INFO,
            format='%(asctime)s - %(levelname)s - %(message)s',
            handlers=[
                logging.FileHandler(os.path.join(log_dir, 'app.log'), mode='w'),
                logging.StreamHandler() # To also show logs in the terminal
            ]
        )
    else:
                    ...
                    if source_type == "SQL File":
                        ...
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            try:
                                ...
                                oracle_logic_summary = llm_result.get('intermediate_outputs', {}).get(task1.description, "No summary available.")
