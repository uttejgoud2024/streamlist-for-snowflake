...
def get_snowpark_session_and_llm():
    ...
    try:
        ...
        if missing_vars:
            raise ValueError(f"Missing Snowflake connection environment variables: {', '.join(missing_vars)}")
...
def run_crew_migration(file_content, source_type, model_type, custom_llm):
    ...
    crew = Crew(agents=[oracle_analyst, dbt_modeler, snowflake_optimizer, quality_reviewer], tasks=tasks, verbose=True)
    ...
    oracle_logic_summary = crew.tasks_outputs[0].result if crew.tasks_outputs else "No summary available."
