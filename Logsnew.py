                ...
                try:
                    ...
                    if source_type == "SQL File":
                        ...
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            status.update(label="✅ **Step 5/5: Final review and validation complete...**", state="running", expanded=True)
...
crew = Crew(agents=[oracle_analyst, dbt_modeler, metadata_generator, snowflake_optimizer, quality_reviewer], tasks=[task1, task2, task3, task4, task5], verbose=True)
                            ...
                            try:
                                ...
                                # Extract results from CrewAI kickoff
                                oracle_logic_summary = crew.tasks_outputs[0]
                                metadata_content_raw = crew.tasks_outputs[2]
                                final_sql_output = crew.tasks_outputs[4]
