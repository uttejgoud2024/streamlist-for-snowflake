...
def run_crew_migration(file_content, source_type, model_type, custom_llm):
    ...
    all_outputs = crew.kickoff()
    logging.info("CrewAI execution completed.")
    
    # Access the outputs from the returned list by index
    # We check if the list is not empty before accessing elements
    if not all_outputs:
        raise ValueError("CrewAI kickoff returned no outputs. Check Snowflake Cortex for errors.")

    oracle_logic_summary = all_outputs[0]
    final_output_str = all_outputs[-1]
