...
def validate_sql(sql_text):
    ...
    try:
        ...
        for statement in parsed:
            ...
            # Only SELECT statements are supported for DBT models
            if statement_type != 'SELECT':
                logging.error(f"Unsupported SQL statement type: {statement_type}.")
                return False, f"Unsupported SQL statement type: {statement_type}. Only SELECT is allowed for DBT models."
...
def create_summary_file(output_dir, file_name, converted_sql, model_type, oracle_logic_summary, metadata_content, explain_plan_content):
    """Creates a summary file with migration details."""
    summary_path = os.path.join(output_dir, f"{os.path.splitext(file_name)[0]}_summary.txt")
    with open(summary_path, "w") as f:
        ...
        f.write("\n--- Converted Snowflake DBT Model ---\n")
        f.write(converted_sql)
        f.write("\n--- Generated DBT Schema.yml ---\n")
        f.write(metadata_content)
        f.write("\n--- Snowflake Query Execution Plan ---\n")
        f.write(explain_plan_content)
...
with tab2:
    ...
    st.code("pip install snowflake-snowpark-python crewai streamlit sqlparse PyYAML")
                ...
                try:
                    ...
                    if source_type == "SQL File":
                        ...
                        with st.status(f"Using CrewAI to convert `{file.name}`...", expanded=True) as status:
                            ...
                            metadata_generator = Agent(role="DBT Metadata Generator", goal="Generate high-quality DBT documentation and tests for the converted Snowflake models.", backstory="An expert in dbt development, specializing in creating `schema.yml` files with detailed column descriptions and appropriate data quality tests (e.g., `not_null`, `unique`). The output must be valid YAML for the `schema.yml` file.", llm=custom_llm, verbose=True)
                            ...
                            # Updated status updates for each step
                            status.update(label="🕵️ **Step 1/5: Analyzing Oracle logic...**", state="running", expanded=True)
                            ...
                            status.update(label="🤖 **Step 2/5: Translating to Snowflake SQL...**", state="running", expanded=True)
                            ...
                            status.update(label="📝 **Step 3/5: Generating DBT metadata and tests...**", state="running", expanded=True)
                            task3 = Task(description="""
                                Based on the converted Snowflake SQL, generate a complete `schema.yml` file.
                                The file should include:
                                - A high-level description for the dbt model.
                                - For each column in the final SELECT statement:
                                    - A detailed description of the column's purpose and logic.
                                    - The appropriate `data_type`.
                                    - Relevant dbt tests (`not_null`, `unique`, etc.).
                                The output must be a valid YAML string for `schema.yml`.
                            """, expected_output="A valid YAML string for `schema.yml` with model and column descriptions and tests.", agent=metadata_generator)
                            
                            status.update(label="⚙️ **Step 4/5: Optimizing query for Snowflake...**", state="running", expanded=True)
                            ...
                            status.update(label="✅ **Step 5/5: Final review and validation...**", state="running", expanded=True)
                            ...
                            crew = Crew(agents=[oracle_analyst, dbt_modeler, metadata_generator, snowflake_optimizer, quality_reviewer], tasks=[task1, task2, task3, task4, task5], verbose=True)
                            
                            try:
                                # Correct method call and access to tasks_outputs
                                crew_output = crew.kickoff()
                                logging.info("CrewAI execution completed.")
                                
                                # Access results from the individual tasks
                                oracle_logic_summary = crew.tasks_outputs[0].result
                                metadata_content_raw = crew.tasks_outputs[2].result
                                final_sql_output = crew.tasks_outputs[4].result

                                # Clean up the final SQL output
                                clean_sql = ""
                                if "```sql" in final_sql_output:
                                    clean_sql = re.search(r"```sql\s*(.*?)\s*```", final_sql_output, re.DOTALL).group(1)
                                elif "```" in final_sql_output:
                                    clean_sql = re.search(r"```\s*(.*?)\s*```", final_sql_output, re.DOTALL).group(1)
                                else:
                                    clean_sql = final_sql_output.strip()
                                
                                converted_sql = convert_oracle_to_snowflake(clean_sql)
                                wrapped_sql = wrap_sql_in_dbt_model(converted_sql, model_type)
                                
                                # Clean up and save the metadata
                                metadata_content_yaml = re.search(r"```yaml\s*(.*?)\s*```", metadata_content_raw, re.DOTALL).group(1)
                                metadata_path = os.path.join(output_dir, f"{safe_name}.yml") # Corrected file name
                                with open(metadata_path, "w") as f:
                                    f.write(metadata_content_yaml)
                                
                                # Generate Explain Plan (optional but highly recommended)
                                explain_plan_content = ""
                                if session:
                                    try:
                                        logging.info("Generating EXPLAIN plan for the query...")
                                        # Use the clean_sql (before wrapping) for the EXPLAIN plan
                                        explain_result = session.sql(f"EXPLAIN {clean_sql}").collect()
                                        explain_plan_content = json.dumps([row.as_dict() for row in explain_result], indent=2)
                                    except Exception as e:
                                        logging.error(f"Failed to generate EXPLAIN plan: {e}")
                                        explain_plan_content = f"Failed to generate EXPLAIN plan: {e}"

                                # Create and save summary file with unique name
                                summary_path = create_summary_file(log_dir, file.name, wrapped_sql, model_type, oracle_logic_summary, metadata_content_yaml, explain_plan_content)
                    ...
                    with open(metadata_path, "rb") as f:
                        st.download_button(label=f"⬇️ Download `{safe_name}.yml`", data=f, file_name=f"{safe_name}.yml", mime="text/yaml")
                    
                    st.success(f"✅ Saved `{safe_name}.sql` and `{safe_name}.yml` to `{output_dir}`")
                except Exception as e:
